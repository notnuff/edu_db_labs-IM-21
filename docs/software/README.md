...
Пояснення:
    ...
    Також у ньому виконується обробка парамертрів командного рядка: Парсер командного рядка може налаштовувати деякі параметри при запуску серверу, у нашому прикладі - можн    <p align="center">
      <img src="./img/comandline.png" alt="Start program">
    </p>а встановити власний порт перед запуском.
...
### Модель сервера і роутингу (server.h)

    Описує клас сервера і методи для обробки різних типів запитів (GET, POST, PATCH, DELETE).

Пояснення:

    У цьому модулі налаштовується власне сервер, який забезпечує підключення до бази даних, налаштовує роутинг шляхів для клієнтських реквестів, і підключає відповідні функції від нашого RESTful api
...
Пояснення
    ...
    Цей модуль забезпечує підключення до бази даних з дефолтними або налаштовуваними параметрами, а також реалізує обробку помилок цього підключення
...
Пояснення:
    ...
    Цей модуль реалізовує відправку і обробку запитів до бази даних, а також обробку і відправку запитів від і до клієнта. Також в його обов'язки входить обробка помилок, якщо такі виникають. Тяжка доля цього API, все таки.
